...
public class IntPrototype extends Prototype{
	private String name;
	private List<Pair> lParam;
	
	public IntPrototype(String name, List<Pair> lParam) {
		this.name = name;
		this.lParam = lParam;
	}
	...
	public RetPrototype toIr(SymbolTable ts) throws TypeException {
         ...
         if(ts.lookup(name) != null) {
        	 throw new TypeException("IntProto -> " + name + " est déjà déclaré");
         }
